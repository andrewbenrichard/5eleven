            ...
            <div class="col-md-12">
              <div class="cotainer">
                <div class="row" v-show="firstloading">
                  ...
                  <span>Loading Naterials</span>
                </div>
                <div class="row" v-if="submaterials.length">
                  <div
                    class="col-md-4"
                    v-for="category in submaterials"
                    :key="category.id"
                  >
                    <router-link :to="'/cat/' + category.slug">
                ...
                <div v-else>
                  <div class="infobox-3">
                    ...
                    <h5 class="info-heading">No sub Materials Found in {{material.material_name}}</h5>
                    <p class="info-text">
          ...
          <div class="modal-dialog" role="document">
            <div class="modal-content">
              <div class="modal-header">
                <h5 class="modal-title" id="MealModalLabel">Add Material to {{material.material_name}}</h5>
                <button
                  type="button"
                  class="close"
                  data-dismiss="modal"
...
export default {
  ...
  methods: {
    ...
    createMaterial() {
      ...
      this.savingBtn = true;
      this.form
        .post("/api/sc_admin/post/sub/material")
        .then(() => {
          Fire.$emit("AfterCreate");
          $("#newMaterial").modal("hide");
          Toast.fire({
            type: "success",
            title: "Sub Material created successfully",
          });
    ...
    loadMaterial() {
      axios
        .get("/api/sc_admin/cat/material/"+ this.$route.params.slug)
        .then(({ data }) => (this.material = data, this.material_id = data.id));
    },
    loadSubMaterials() {
      axios
        .get("/api/sc_admin/single/material/sub/" + this.$route.params.slug)
